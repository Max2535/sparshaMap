...
import { actionCreators } from '../store/promotion';
    ...
    deleteData(name) {
        const getAlert = () => (
            <SweetAlert
                ...
                title={<h4><span>คุณต้องการลบ Package <b>{name}</b>!หรือไม่</span></h4>}
                ...
                {this.props.isLoading ?
                    ...
                    <div>
                        <table className='table'>
                            ...
                            <tbody>
                                {this.props.jsonDataPromotion.map((data, index) =>
                                    <tr key={this.props.startDateIndex + index + 1}>
                                        <td>{this.props.startDateIndex + index + 1}</td>
                                        <td>{data.PACKAGE_NAME}</td>
                                        <td><Moment format="DD/MM/YYYY">
                                            {data.EFF_DATE}</Moment></td>
                                        <td><Moment format="DD/MM/YYYY">{data.END_DATE}</Moment>{' '}</td>
                                        <td><Button onClick={this.toggle} color="warning">แก้ไข</Button>{' '}
                                            <a onClick={() => this.deleteData(data.PACKAGE_NAME)} className='btn btn-danger'>
                        ...
                        <p className='clearfix'>
                            ...
                            <div className="text-right">
                                {console.log("prevStartDateIndex:"+prevStartDateIndex)}
                                {prevStartDateIndex > -1 ? <Link className='btn btn-default pull-left' to={`/Promotion/${prevStartDateIndex}`}><Button color="primary">ก่อนหน้า</Button></Link> : ""}
                                {/*ต้อง Check จาก total page*/}
                                <Link className='btn btn-default pull-right' to={`/Promotion/${nextStartDateIndex}`}><Button color="primary">ถัดไป</Button></Link>
                            </div>
                        </p>
                    </div>
                }
